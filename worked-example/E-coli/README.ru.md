...
| Файл | Назначение |
|------|------------|
| `eta_L_ecoli.py` | Центральный расчёт: $\eta_L$ и $\eta_L^{\text{comp}}$ через эксергический и вычислительный знаменатели; печатает таблицу результатов |
| `sensitivity.py` | Анализ чувствительности методом Монте-Карло: разброс $\eta_L$ по диапазонам $I_{\text{pred}}, E_{\text{actual}}, \eta_{\text{ex}}, T$ |
| `expected_output.txt` | Эталонный вывод `eta_L_ecoli.py` для верификации воспроизводимости |

Использует только стандартную библиотеку Python (`math`, `statistics`); внешних зависимостей нет.
...
Вывод `eta_L_ecoli.py` должен совпадать с `expected_output.txt` с точностью до ошибок округления чисел с плавающей точкой. Числовые узлы:

- $N_{\text{max}}^{\text{ex}} \approx 3 \cdot 10^{11}$ бит на клетку (эксергический бюджет);
- $N_{\text{max}}^{\text{comp}} \approx 3 \cdot 10^{8}$ бит на клетку (вычислительный, $\eta_{\text{ex}}^{\text{comp}} = 10^{-3}$);
- $\eta_L \approx 3 \cdot 10^{-8}$ через эксергический знаменатель;
- $\eta_L^{\text{comp}} \approx 3 \cdot 10^{-5}$ через вычислительный знаменатель.
...
**(г) Оценка взаимной информации $I_{\text{pred}}$** — литературные оценки сходятся на $I_{\text{pred}} \in [10^{3}, 10^{4}]$ бит на клетку за поколение, центральная оценка $10^{4}$ бит [ShimizuTuBerg2010, Cheong2011, MehtaSchwab2012]. В анализе чувствительности варьируется по полному диапазону.
...
Вычислительный знаменатель использует $\eta_{\text{ex}}^{\text{comp}} \approx 10^{-3}$ — долю эксергии, идущую на необратимые информационные операции; остальное расходуется на синтез биомассы и ионные градиенты [MehtaSchwab2012, Lan2012].
