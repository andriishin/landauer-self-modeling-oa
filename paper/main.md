...
**relative** contribution of each component to the total dissipation of the connected system; successive refinement, stable under permutations of the subtracted elements, yields a justified boundary. This is not a complete elimination of conventionality
...
weights $w_T = w_C = w_S = 1$ are a convention; for unequal weights (with $w_i > 0$) $I_v^{(w)} = (\tilde{T}_v^{w_T} \cdot \tilde{C}_v^{w_C}
...
Refutation of any one capacity collapses both scales at once — the vitality verdict $V(X) = S \wedge (\eta_v > 0)$ and the structural screening $I_v$: a failure of $C_v$ acts through $\eta_v$ (via $I_{\text{mem}}$ or $I_{\text{pred}}$), a failure of $T_v$ or of the error-return loop $S_v$ acts through the predicate $S$, and any of the three collapses $I_v$ through the geometric mean. Refutations of the two
...
Refutation of any capacity simultaneously collapses both scales — the verdict $V(X)$ (through $\eta_v$ if the failed capacity is $C_v$, through the predicate $S$ if it is $T_v$ or $S_v$) and the screening index $I_v$ through the geometric mean. This is one falsification through capacity, not a double one.
